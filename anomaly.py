...
class probabilisticEWMA:
    ...
    mydict = {"s1": 0, "s2": 0, "tflag": False, "count": 0}
    ...
    def setMovingParameters( self, s1=0, s2=0, tflag=False, count=0 ):
        #check if term exist in the store
        read_dict = redis.get(self.term)
        if read_dict:
            mydict = {"s1": s1, "s2": s2, "tflag": tflag, "count": count}
    ...
    def  calcAnomalyScore(self): 
        ...
        self.setMovingParameters( s1=self.s1, s2=self.s2, tflag=True, count=count )   
        return prob_List
